...
fn test_sumcheck_cross_layered_helper<C: GKRFieldConfig>() {
    ...
    let inputs = (0..circuit.layers[0].layer_size).map(|_| C::SimdCircuitField::random_unsafe(&mut rng)).collect::<Vec<_>>();
    ...
    let (_output_claim, _input_challenge, _input_claim) = prove_gkr(&circuit, &evals, &connections, &mut transcript, &mut sp);
