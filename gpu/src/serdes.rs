...
use circuit::Circuit;
use gkr_engine::{FieldEngine, Transcript, ExpanderDualVarChallenge};
...
where C::SimdCircuitField: std::fmt::Debug {
    // Get element size for auxiliary decision making
...
pub fn serialize_circuit_to_file<C: FieldEngine>(circuit: &Circuit<C>, filepath: &str) -> Result<(), SerializationError> 
where C::CircuitField: std::fmt::Debug, C::SimdCircuitField: std::fmt::Debug {
    ...
    for (layer_idx, layer) in circuit.layers.iter().enumerate() {
        // Write layer header: Layer [num_gate_add] [num_gate_mul] [input_var_num] [output_var_num] [input_vals_count] [output_vals_count]
        ...
        for add_gate in &layer.add {
            ...
            writeln!(writer, "Add {} {} {}", add_gate.i_ids[0], add_gate.o_id, coef_str)?;
        ...
        for mul_gate in &layer.mul {
            ...
            writeln!(writer, "Mul {},{} {} {}", 
                mul_gate.i_ids[0], mul_gate.i_ids[1], 
                mul_gate.o_id, coef_str)?;
        ...
        let percent = if total_items > 0 { (items_processed * 100) / total_items } else { 100 };
        if percent > last_percent && percent % 5 == 0 {
            println!("Serialization progress: {}% (processed {}/{} items: {} gates + {} values)", 
    ...
    match std::fs::metadata(filepath) {
        Ok(metadata) => {
            ...
            println!("Successfully serialized {} gates and {} values to file (total {} items)", 
...
where F::CircuitField: std::fmt::Debug, F::SimdCircuitField: std::fmt::Debug {
    ...
    if std::path::Path::new(&filepath).exists() {
        println!("Circuit file {} already exists, skipping serialization", filepath);
        return Ok(());
    }
    ...
    for (i, chunk) in digest_bytes.chunks(40).enumerate() {
        ...
        let formatted_bytes: Vec<String> = chunk.iter()
    ...
    println!("Total digest bytes: {}, Total proof bytes: {}, Hash start index: {}", 
             digest_bytes.len(), proof_bytes.len(), hash_start_index);
