...
fn hyrax_committing_benchmark_helper(
    ...
    for num_vars in lowest_num_vars..=highest_num_vars {
        ...
        let srs =
            HyraxPCS::<G1Affine, BytesHashTranscript<Keccak256hasher>>::gen_srs_for_testing(
                &num_vars, &mut rng,
            );
        ...
        group
            .bench_function(
                ...
                |b| {
                    b.iter(|| {
                        _ = black_box(
                            HyraxPCS::<G1Affine, BytesHashTranscript<Keccak256hasher>>::commit(
                                &num_vars,
                                &srs,
                                &poly,
                                &mut scratch_pad,
...
fn hyrax_opening_benchmark_helper(
    ...
    for num_vars in lowest_num_vars..=highest_num_vars {
        ...
        let _ = HyraxPCS::<G1Affine, BytesHashTranscript<Keccak256hasher>>::commit(
            &num_vars,
            &srs,
            &poly,
            &mut scratch_pad,
