//! Implement small scalar multi-exponentiation for G1 and G2 points.
//! Assumptions: the scalars are less than 16 bits or 32 bits respectively.
//!
//! Code adapted from Halo2's multiexp_serial function
use std::ops::Neg;

use halo2curves::{ff::PrimeField, group::Group, CurveAffine};
...
/// Multi-exponentiation with scalars less than 16 bits.
pub fn msm_serial_16bits<C: CurveAffine>(coeffs: &[C::Scalar], bases: &[C], acc: &mut C::Curve) {
    let coeffs_u16 = coeffs
        .iter()
        .map(|a| a.to_repr().as_ref()[..2].to_vec())
        .collect::<Vec<_>>();
    ...
    for current_window in (0..number_of_windows).rev() {
        ...
        for (coeff, base) in coeffs_u16.iter().zip(bases.iter()) {
