...
#[inline(always)]
pub fn _eq(x: &FPrimitive, y: &FPrimitive) -> FPrimitive {
    // x * y + (1 - x) * (1 - y)
    let xy = *x * y;
...
pub fn _eq_eval_at(
    ...
    mul_factor: &FPrimitive,
    eq_evals: &mut [FPrimitive],
    sqrt_n_1st: &mut [FPrimitive],
    sqrt_n_2nd: &mut [FPrimitive],
) {
    ...
    _eq_evals_at_primitive(&r[first_half_bits..], &FPrimitive::one(), sqrt_n_2nd);
    ...
    fn poly_eval_at(
        ...
        for i in 0..eval_size {
            ...
            for j in 0..M31_VECTORIZE_SIZE {
                let f_v_0 = src_v[i * 2].v[j];
                let f_v_1 = src_v[i * 2 + 1].v[j];
        ...
        for i in 0..self.cur_eval_size >> 1 {
            if !gate_exists[i * 2] && !gate_exists[i * 2 + 1] {
                ...
                for j in 0..M31_VECTORIZE_SIZE {
                    if var_idx == 0 {
                        bk_f[i].v[j] =
                            init_v[2 * i].v[j] + (init_v[2 * i + 1].v[j] - init_v[2 * i].v[j]) * r;
                    } else {
                        bk_f[i].v[j] =
                            bk_f[2 * i].v[j] + (bk_f[2 * i + 1].v[j] - bk_f[2 * i].v[j]) * r;
                    }
                }
                bk_hg[i] = F::zero();
            } else {
                ...
                for j in 0..M31_VECTORIZE_SIZE {
                    ...
                    bk_hg[i].v[j] =
                        bk_hg[2 * i].v[j] + (bk_hg[2 * i + 1].v[j] - bk_hg[2 * i].v[j]) * r;
                }
            }
        }
...
impl<'a> SumcheckGkrHelper<'a> {
    ...
    pub fn receive_challenge(&mut self, var_idx: usize, r: FPrimitive) {
        if var_idx < self.input_var_num {
