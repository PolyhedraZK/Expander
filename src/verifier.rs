...
#[inline]
fn degree_2_eval<C: GKRConfig>(p0: C::ChallengeField, p1: C::ChallengeField, p2: C::ChallengeField, x: C::ChallengeField) -> C::ChallengeField {
    if C::FIELD_TYPE == FieldType::GF2 {
        let c0 = &p0;
        let c2 =
            (p2 - p0 - p1.mul_by_x() + p0.mul_by_x()) * (C::ChallengeField::X - C::ChallengeField::one()).mul_by_x().inv().unwrap();
        let c1 = p1 - p0 - c2;
        *c0 + (c2 * x + c1) * x
    } else {
        let c0 = &p0;
...
#[inline]
fn degree_3_eval<C: GKRConfig>(p0: C::ChallengeField, p1: C::ChallengeField, 
    p2: C::ChallengeField, p3: C::ChallengeField, x: C::ChallengeField) -> C::ChallengeField {
    // TODO-OPTIMIZATION: precompute values and inverses 
    if C::FIELD_TYPE == FieldType::GF2 {
        lag_eval(&[C::ChallengeField::zero(), C::ChallengeField::one(), C::ChallengeField::X, C::ChallengeField::X.mul_by_x()], &[p0, p1, p2, p3], &x)
    } else {
        lag_eval(&[C::ChallengeField::zero(), C::ChallengeField::one(), C::ChallengeField::from(2), C::ChallengeField::from(3)], &[p0, p1, p2, p3], &x)
    }
}
...
fn eval_sparse_circuit_connect_poly<C: GKRConfig, const INPUT_NUM: usize>(
    ...
    eq_evals_at_primitive(r_simd_xy, &C::ChallengeField::one(), &mut eq_evals_at_r_simd_xy);
    ...
    if INPUT_NUM == 0 {
        ...
        for cst_gate in gates {
            v += C::challenge_mul_circuit_field(
                &(eq_evals_at_rz0[cst_gate.o_id] + eq_evals_at_rz1[cst_gate.o_id]), &cst_gate.coef);
        }
