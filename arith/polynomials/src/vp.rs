...
impl<F: Field> VirtualPolynomial<F> {
    ...
    pub fn evaluate(&self, point: &[F]) -> F {
        ...
        let evals: Vec<F> = self
            ...
            .iter()
            .map(|x| x.eval_reverse_order(point))
            .collect();
    ...
    pub fn rand_zero<R: RngCore>(
        ...
        for _ in 0..num_products {
            let num_multiplicands =
                rng.gen_range(num_multiplicands_range.0..num_multiplicands_range.1);
            let product = random_zero_mle_list(nv, num_multiplicands, &mut *rng);
            let coefficient = F::random_unsafe(&mut *rng);
            poly.add_mle_list(product.into_iter(), coefficient);
        }
    ...
    pub fn print_evals(&self) {
        ...
        for i in 0..1 << self.aux_info.num_variables {
            ...
            let point_fr: Vec<F> = point.iter().map(|&x| F::from(x as u32)).collect();
...
pub fn eq_eval<F: Field>(x: &[F], y: &[F]) -> F {
    if x.len() != y.len() {
        panic!("x and y have different length {} vs {}", x.len(), y.len());
    }
...
mod test {
    ...
    fn test_eq_xr() {
        ...
        for nv in 4..10 {
            ...
            let eq_x_r3 = crate::EqPolynomial::build_eq_x_r(r.as_ref());
            assert_eq!(eq_x_r, eq_x_r2);
            assert_eq!(eq_x_r.coeffs, eq_x_r3);
        }
    }
    ...
    // over r, which is
    //      eq(x,y) = \prod_i=1^num_var (x_i * r_i + (1-x_i)*(1-r_i))
    fn build_eq_x_r_for_test<F: Field>(r: &[F]) -> Arc<MultiLinearPoly<F>> {
        // we build eq(x,r) from its evaluations
        // we want to evaluate eq(x,r) over x \in {0, 1}^num_vars
        // for example, with num_vars = 4, x is a binary vector of 4, then
        //  0 0 0 0 -> (1-r0)   * (1-r1)    * (1-r2)    * (1-r3)
        ...
        let mle = MultiLinearPoly { coeffs: eval };
