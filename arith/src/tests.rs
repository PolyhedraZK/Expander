...
pub fn random_simd_field_tests<Scalar:Field + Send, F: SimdField<Scalar>>(_name: String) {
    let mut rng = test_rng();

    {
        let a = F::random_unsafe(&mut rng);
        let s1 = Scalar::random_unsafe(&mut rng);
        let s2 = Scalar::random_unsafe(&mut rng);
...

    {
        let a = F::random_unsafe(&mut rng);
        let b = F::random_unsafe(&mut rng);
        let s = Scalar::random_unsafe(&mut rng);
