...
impl FieldSerde for AVXM31 {
    ...
    fn deserialize_from<R: Read>(mut reader: R) -> FieldSerdeResult<Self> {
        ...
        unsafe {
            let mut value = transmute::<[u8; Self::SERIALIZED_SIZE], __m256i>(data);
...
impl Field for AVXM31 {
    ...
    const ZERO: Self = Self { v: [PACKED_0, PACKED_0] };

    const ONE: Self = Self {
        v: unsafe { transmute::<[u32; 16], [__m256i; 2]>([1; M31_PACK_SIZE]) },
    };

    const INV_2: Self = Self { v: [PACKED_INV_2, PACKED_INV_2] };
    ...
    fn is_zero(&self) -> bool {
        ...
        unsafe {
            let pcmp = _mm256_cmpeq_epi32_mask(self.v[0], PACKED_0) & _mm256_cmpeq_epi32_mask(self.v[1], PACKED_0);
            let pcmp2 = _mm256_cmpeq_epi32_mask(self.v[0], PACKED_MOD) & _mm256_cmpeq_epi32_mask(self.v[1], PACKED_MOD);
            (pcmp | pcmp2) == 0xFF
        }
    }
    ...
    fn mul_by_5(&self) -> AVXM31 {
        let double = unsafe { [mod_reduce_epi32(_mm256_slli_epi32::<1>(self.v[0])), mod_reduce_epi32(_mm256_slli_epi32::<1>(self.v[1]))] };
        let quad = unsafe { [mod_reduce_epi32(_mm256_slli_epi32::<1>(double[0])), mod_reduce_epi32(_mm256_slli_epi32::<1>(double[1]))] };
        let res = unsafe { [mod_reduce_epi32(_mm256_add_epi32(self.v, quad[0])), mod_reduce_epi32(_mm256_add_epi32(self.v, quad[1]))] };
        Self { v: res }
    }
    ...
    fn from_uniform_bytes(bytes: &[u8; 32]) -> Self {
        ...
        Self {
            v: unsafe { [_mm256_set1_epi32(m.v[0] as i32), _mm256_set1_epi32(m.v[1] as i32)] },
        }
    }
    ...
    fn mul_by_3(&self) -> AVXM31 {
        let double = unsafe { [mod_reduce_epi32(_mm256_slli_epi32::<1>(self.v[0])), mod_reduce_epi32(_mm256_slli_epi32::<1>(self.v[1]))] };
        let res = unsafe { [mod_reduce_epi32(_mm256_add_epi32(self.v[0], double)), mod_reduce_epi32(_mm256_add_epi32(self.v[1], double))] };
        Self { v: res }
    }
}
...
impl Debug for AVXM31 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        unsafe {
            ...
            _mm256_storeu_si256(data.as_mut_ptr().offset(8) as *mut __m256i, self.v[1]);
        }
